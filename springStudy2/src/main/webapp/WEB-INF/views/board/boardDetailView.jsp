...
<body>
                    ...
                    <!-- to get an API token!-->
                   	...
                   	<c:if test="${sessionScope.login.memId == board.memId}">
                   		<form id="boardForm" action="<c:url value="/boardEditView"/>" method="post"> 
                   			...
                   			<input class="btn btn-warning btn-lx" type="submit" name="action" value="수정"></button>
                   			<input class="btn btn-danger btn-lx" type="submit" name="action" value="삭제"></button>
                   		</form>
                   	</c:if>
                </div>
            </div>
    ...
    <script>
    	...
    	function fn_write(){
   			...
   			var sendData = JSON.stringify({"memId":memId, "boardNo":boardNo, "replyContent":msg});
   			...
   			$.ajax({
   				...
   				,success:function(res){
   					...
   					$("#replyInput").val("");
   					var str = "";
   					str += "<tr id='" + reply.replyNo + "'>";
   					str += "<td>" + res.replyContent + "</td>";
   					str += "<td>" + res.memNm + "</td>";
   					str += "<td>" + res.replyDate + "</td>";
   					str += "<td><a onclick='fn_del(\"" + reply.replyNo + "\")' >X</a></td>";
   					str += "</tr>";
